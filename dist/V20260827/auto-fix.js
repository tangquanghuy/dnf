(function () {
    ...
    // thinking/think 属于思考域，误生成在其中的美化卡片不应保留。
    // 成对卡片会连同卡片内容一起删除；残缺的单个开/闭标签也会被清掉。
    function removeBeautifyCardsFromThinking(text) {
        return String(text || '').replace(
            /(<\s*(?:thinking|think)\s*>)([\s\S]*?)(<\s*\/\s*(?:thinking|think)\s*>)/gi,
            (match, openTag, inner, closeTag) => {
                ...
                for (const tagName of BEAUTIFY_CARD_TAG_NAMES) {
                    ...
                    const strayCardTagRe = new RegExp(
                        `<\\s*\\/?\\s*${tagName}\\b[^>]*>`,
                        'gi',
                    );
                    ...
                    cleanedInner = cleanedInner
                        .replace(cardBlockRe, '\n')
                        .replace(strayCardTagRe, '');
                }

                if (cleanedInner === inner) {
                    return match;
                }
                return `${openTag}${collapseBlankLines(cleanedInner)}${closeTag}`;
            },
        );
    }
    ...
    function normalizeTagBlocks(message) {
        ...
        // 先清掉思考域内误生成的卡片，再处理正文标签和代码围栏。
        const thinkingCleaned = removeBeautifyCardsFromThinking(original);
        // 先把 now_plot 内误用的代码围栏 ``` 移出提取范围，再清理正文八股词。
        let deFenced = evictNowPlotCodeFences(thinkingCleaned);
